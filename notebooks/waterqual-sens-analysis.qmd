---
title: "Water Quality Sensitivity Analysis"
...
grid_out <- tidyr::crossing(Retention=0:1, Nitrates=0:1)[-1,]
```
...
```{r}
#| label: "water quality optimization series"
#| eval: false

# create full solution to compare to
solution_all <- generate_optim(inputs=watqual_rasts[[which(grid_out[nrow(grid_out),] == 1)]],
...
for(i in 1:(nrow(grid_out)-1)) {
  config <- generate_optim(inputs=watqual_rasts[[which(grid_out[i,] == 1)]],
                           cost=cost.mil, targets=seq(0.05,0.75,0.05),
                           output.rasters=TRUE, output.table=FALSE,
                           force.solve = TRUE)
...
write.csv(emd_series, "../data/water_quality/sensitivity_analysis/emd_series.csv",
          row.names = FALSE)
```
...
```{r}
#| label: fig-watqual-emd

emd_series <- read.csv("../data/water_quality/sensitivity_analysis/emd_series.csv")

grid_out$config <- 1:nrow(grid_out)

emd_series <- left_join(emd_series, grid_out) %>%
  mutate(ninputs = rowSums(across(Retention:Nitrates)))
...
low_auc_series <- low_auc_series %>%
  mutate(`Leaves Out` = str_replace_all(leaves_out, "Retention", "N Retention Capacity"),
         `Leaves Out` = str_replace_all(`Leaves Out`, "Nitrates", "Nitrate Rank")) %>%
  select(-leaves_out) %>%
  rename("AUC" = "auc")
