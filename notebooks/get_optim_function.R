...
# Inputs:
  # inputs: A stack of terra rasters of ecosystem services maps (units in ES per pixel). Optimized solutions will be generated that target a specified amount of these ES.
  # cost: A terra raster of cost per pixel. Fed directly into optimization problem with no modification
  # targets: A vector (or single number) specifying protection target(s) to be applied to the ES layers. A vector will cause multiple optimizations to be run. All ES will have the same target applied to them. A matrix may be used where each row is an optimization run and each column is an ES from the inputs raster stack. This will be used in add_relative_targets() in prioritizr
  # mask: An optional terra raster. If table.columns includes "percent_mask_pixels_in_solution", the percent of pixels in the solution that are within the mask layer will be calculated.
  ...
  # force.solve: force prioritizr to solve problem even if pre-solution checks fail
...
# 2023-04-19: Converted from raster to terra package functionality
# 2024-05-07: Added option to force solve when failing pre-solve check
# 2024-05-21: Changed pre-processing step so that layers' max < 100 instead of <1000
###################################
...
generate_optim <- function(inputs, cost, targets, mask=NULL, 
  ...
  for(i in 1:nlyr(inputs)) {
    ...
    #divide layers so sum of the largest target is <1e6
    if(sum(values(inputs[[i]]), na.rm=TRUE)*max(targets) > 1e6) {
      digits_before_decimal <- floor(log10(max(inputs[[i]][],na.rm=TRUE))) + 1
      divide_by <- 1 * 10^(digits_before_decimal - 1)
  ...
  for(i in 1:NROW(targets)) { #generate solutions for the range of protection targets
    ...
    solution <- solve(problem, force = force.solve) #use Gurobi to find optimal solution
